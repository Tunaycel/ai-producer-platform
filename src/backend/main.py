...
def health_check():
    ...
    return {"status": "online", "engine": "AI Producer Master Engine v1.0", "quality": "Pro Studio Grade"}
...
def producer_chat(payload: dict):
    ...
    reference_meta = payload.get("reference_meta", None)
    ...
    result = ProducerAIService.process_artist_request(
        prompt=message,
        vocal_present=has_vocal,
        reference_audio_meta=reference_meta
    )
...
async def analyze_reference(file: UploadFile = File(...)):
    ...
    analysis = AudioAnalyzer.detect_bpm_and_key(filename=file.filename, sample_data=content)
